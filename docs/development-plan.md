...
- `/projects`、`/create`、`/list`、`/activate`、`/send`、`/screen`、`/read`、`/attach`、`/attach-last`、`/enablePermission`、`/kill`、`/sys`。
...
| `TASK-107` | `FR-006` | 历史对话续聊 | 支持 attach、attach-last、直接历史会话编号续聊 | `src/codex-adapter.js`、`src/automation-agent.js` | 已完成 |
| `TASK-108` | `FR-007` | 列表与通知 | 支持 `/list` 聚合展示、编号模式、通知回传 | `src/automation-agent.js`、`src/telegram-notifier.js` | 已完成 |
| `TASK-109` | `FR-008` | 配置层 | 支持白名单目录、等待参数、驱动与系统模式配置 | `src/default-config.js`、`src/telegram-entry.js` | 已完成 |
| `TASK-110` | `NFR-004` | 服务与日志 | 常驻服务、日志目录、异常通知、环境加载 | `src/service-entry.js`、`src/file-logger.js` | 进行中 |
| `TASK-111` | `NFR-001` | 跨平台验证 | 补齐 macOS 真实环境验证与文档回写 | 文档、测试记录 | 待开始 |
| `TASK-112` | `FR-003` | 完成态识别 | 继续增强复杂输出场景下的结果判定 | `src/output-analysis.js`、测试 | 进行中 |
| `TASK-113` | `FR-007` | 系统联动 | 对真实系统动作做更严格的生产验证 | `src/system-adapter.js`、验收文档 | 暂缓 |
...
| `M4` | 文档与测试统一 | 核心文档、README、状态摘要、自动化测试口径一致 | 已完成 |
| `M5` | 服务化硬化 | 补齐常驻服务、日志、异常通知与部署说明 | 进行中 |
| `M6` | 跨平台扩展 | 完成 macOS 真实验证并回写结果 | 待开始 |
...
| 系统适配器 | 通过基础自动化 | `tests/system-adapter.test.js` |
...
3. `/send`、`/screen`、`/list`、`/activate` 等核心交互已统一到文档、实现和测试中。
