...
当前设计服务于“Telegram 远程控制 Codex CLI 会话”这条主线。系统不是 VS Code GUI 自动化工具，而是把以下人工流程抽象成可远程调度的逻辑会话系统：
...
1. 接入层 `Remote Interface`
   ...
   - WeCom 为兼容保留入口
   - 负责接收消息、回传通知、拆分长消息
2. 命令层 `Parser / Policy / Routing`
   - 解析 slash command
   - 把普通文本路由到当前聊天绑定的活动会话
   - 执行来源授权与参数校验
3. 编排层 `Automation Agent`
   - 统一调度会话、任务、通知、系统适配器和 Codex 适配器
   - 负责 `/send`、`/screen`、`/list`、`/activate`、`/help` 等核心命令流程
4. 执行层 `Codex Adapter`
   - 默认主路径为 `exec-json`
   - 兼容 `pty` 与 `pipe` 驱动，用于排障或平台兼容
...
- 职责：解析 `/create`、`/list`、`/send` 等命令；校验来源是否授权
- 输入：消息文本、来源标识、来源绑定信息
- 输出：标准化命令对象
...
- 职责：创建任务、记录事件、分发命令、格式化回复、发送通知、更新会话绑定
...
- 职责：创建会话、发送 prompt、恢复历史会话、读取历史索引、终止进程、读取文件
- 输入：会话元数据、prompt、驱动模式、工作目录、执行档位
- 输出：结构化执行结果、Codex thread id、输出事件、退出事件
- 依赖：`codex-adapter.js`、本机 `codex` CLI、`node-pty`

### 4.8 输出分析层

- 职责：区分 assistant 输出、prompt echo、UI 状态行、终端装饰线，辅助判断“当前轮是否完成”
- 输入：输出缓冲行
...
- 职责：保存会话、任务、来源绑定；维护输出 cursor 和缓冲窗口
...
- 职责：服务启动时恢复状态、清理残留活跃进程、把未完成任务标记为 `interrupted`
...
- 职责：返回 `/sys` 系统摘要、执行可选系统动作、提供服务化入口与日志
- 输入：系统命令、会话统计、环境变量
- 输出：系统状态、关机计划结果、日志记录
...
### 5.5 `/help` 与 `/projects` 流程

1. `/help` 读取当前运行配置
2. 帮助文本生成器把允许根目录、默认等待时间、执行档位、sandbox 与命令列表拼装成可直接复制的说明
