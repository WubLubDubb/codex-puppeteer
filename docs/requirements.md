...
- 创建日期：`2026-03-20`
- 更新日期：`2026-03-24`
- 当前状态：当前主线已统一为“Telegram 远程控制 Codex CLI 会话”的 Windows 优先版本；WeCom 为保留兼容模块，非当前个人交付主路径。

## 2. 项目背景与目标

本项目面向个人开发者，目标是把“在本机项目目录中打开终端，输入 `codex`，持续对话开发”的人工流程，抽象为一个可以被手机远程控制的自动化代理。

当前真实目标不是控制 VS Code GUI，也不是去点击编辑器里的发送按钮，而是直接控制本机 `Codex CLI`：
...
- `/send` 在常规场景下可以直接返回当前轮结果，必要时还能继续通过 `/screen` 查看后续输出。
...
- 支持 `/projects`、`/create`、`/list`、`/activate`、`/send`、`/screen`、`/read`、`/attach`、`/attach-last`、`/enablePermission`、`/kill`、`/sys` 等命令。
...
| `/attach -last -w <workspace> [-n <name>]` | 绑定最近一次本机 Codex 历史对话 |
| `/attach -i <codexSessionId> -w <workspace> [-n <name>]` | 绑定指定本机 Codex 历史对话 |
...
| `/enablePermission -n <sessionId|listNumber>` | 切换会话权限模式 |
| `/kill -n <sessionId|listNumber>` | 终止指定会话 |
| `/sys` | 查看宿主机与运行时状态 |
| `/shutdown ...`、`/cancel_shutdown` | 高风险系统动作，当前非主验收路径 |

> 说明：`/wait` 不再是推荐用户指令。当前设计要求用户以 `/send` 为主交互入口，必要时再用 `/screen` 查看后续输出。
...
- 名称：支持会话权限切换与紧急终止
- 描述：系统必须支持切换会话的权限模式，并在需要时终止指定会话，避免远程任务持续占用资源或进入不可控状态。
...
- 描述：系统必须持久化逻辑会话、任务和来源绑定；支持通过 `/attach`、`/attach-last` 或 `/send -n <codexConversationId>` 继续本机已有 Codex 历史对话；服务重启时要恢复可继续使用的逻辑会话，并把未完成任务标记为中断。
...
- 名称：回传状态并支持多会话远程管理
- 描述：系统必须能够回传任务开始、即时 `/screen` 提示、最终结果、异常错误等通知；支持通过 `/list` 同时查看托管会话和本机历史会话，并支持 `activate` 切换当前聊天绑定会话；支持 `/sys` 查看宿主机状态。高风险系统动作可保留，但应明确与主控制链路隔离。
- 输入：查询命令、任务事件、系统探针信息
- 输出：列表文本、编号选择、通知文本、系统状态摘要
- 前置条件：通知模块与系统探针可用
...
- 描述：系统必须允许维护者通过环境变量配置项目根目录白名单、Telegram 参数、Codex CLI 路径、驱动模式、发送等待时间、系统动作模式等，使不同主机可以复用同一套代码；`/projects` 应帮助用户发现允许目录下的项目，减少手输路径成本。
...
- 类别：安全性
- 描述：系统必须限制允许来源、限制允许项目根目录，且不得把 `.env`、bot token、敏感密码等内容暴露到远程回复或提交产物中。
...
- 对应需求：`FR-005`
- 验收条件：`/enablePermission` 和 `/kill` 可以正确变更会话状态。
- 验收方式：自动化测试验证状态切换和终止结果。

### AC-006

- 对应需求：`FR-006`
- 验收条件：系统重启后可以恢复已持久化会话；可以通过 attach 或直接历史会话编号继续已有对话。
- 验收方式：持久化与恢复自动化测试。

### AC-007

- 对应需求：`FR-007`
- 验收条件：`/list` 能同时展示托管会话与本机历史会话，并支持编号操作；`/activate` 能切换当前聊天绑定会话；`/sys` 能返回系统摘要。
- 验收方式：自动化测试与实际消息回传检查。

### AC-008

- 对应需求：`FR-008`
- 验收条件：不同主机可通过配置切换允许根目录、Bot 参数、Codex CLI 路径和等待策略；`/projects` 可列出允许目录下的项目目录。
- 验收方式：环境变量配置测试、`/projects` 自动化测试与启动验证。
