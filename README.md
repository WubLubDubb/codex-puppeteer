...
当前 `/help`、`/projects`、`/list`、`/current` 都会附带导航按钮，`/projects` 与 `/list` 在存在活动会话时还会附带 `输出`、`模式` 快捷入口。
`/projects`、`/list`、`/ls`、`/find` 的按钮结果现在支持 `上一页 / 下一页` 分页；如有需要，也可以手动输入 `-pg <页码>`。
按钮文案现在尽量采用“动作 + 对象”的方式，例如 `创建会话: 项目名`、`切换到会话 3`、`下载文件: README.md`，降低手机端理解成本。
列表类按钮也会尽量按“当前页 / 分区 / 即时操作 / 全局导航”分层展示，减少只看按钮时的理解负担。
对于 `/projects`、`/list`、`/ls`、`/find`、`/current`、`/mode` 这类主要导航按钮，Telegram 现在会优先原地编辑已有消息，而不是额外再刷一条新消息。
...
- `/list -pg <页码>`
  翻到会话按钮结果的指定页；如果你先用了 `/list -a` 或 `/list -c`，分页按钮会自动保持该上下文。
...
`/ls` 与 `/find` 的结果会附带按钮：目录可直接点进，文件可直接下载；当条目较多时也可以直接用翻页按钮继续浏览。
